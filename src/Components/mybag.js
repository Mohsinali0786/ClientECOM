...
const Mybag = () => {
    ...
    const [dataFromLSStateMen, setdataFromLSStateMen] = useState([])
    const [dataFromLSStateKids, setdataFromLSStateKids] = useState([])
    ...
    let datafromLS_Men = undefined
    let datafromLS_Kids = undefined
    ...
    useEffect(() => {
        ...
        datafromLS_Men = (JSON.parse(localStorage.getItem('Men')))
        datafromLS_Kids = (JSON.parse(localStorage.getItem('Kids')))
        setdataFromLSState(datafromLS)
        setdataFromLSStateMen(datafromLS_Men)
        setdataFromLSStateKids(datafromLS_Kids)
    }, [updateddata])
    ...
    const increment = (item_type, index) => {
        ...
        if (item_type === 'Women') {
        ...
        else if (item_type === 'Men') {
            dataFromLSStateMen?.items.map((myitems, i) => {
                console.log('My items-------------------- ', myitems)
                if (index !== myitems.id) {
                    console.log('If running')
                    ...
                    console.log('Updated my items', myitems)
                    updateddata = myitems
                    dataFromLSStateMen.items[index] = updateddata
                    myquan = 1
                    console.log('My Quan set to initial val', myquan)

                }
                localStorage.setItem('Men', JSON.stringify(dataFromLSStateMen))
            ...
            dataFromLSStateKids?.items.map((myitems, i) => {
                ...
                if (index !== myitems.id) {
                    ...
                    console.log('Updated my items', myitems)
                    updateddata = myitems
                    dataFromLSStateKids.items[index] = updateddata
                    myquan = 1
                    console.log('My Quan set to initial val', myquan)
    ...
    const decrement = (item_type, index) => {
        console.log("Increment function====>", index)
        let myquan = 1
        console.log("My Quan", myquan)
        console.log(datafromLS, "________________________________")
                                    ...
                                    <div className="update-quantity" >
                                        <div className="D-quan-btn">
                                            <Button className="quan-btn" variant="contained" color='success'
                                                onClick={() => decrement(item.type, index)}
                                            >
                                                -
                                            </Button>
                                            <Button className="quan-btn" variant="contained" color='success'
                                ...
                                <>
                                    ...
                                    <div className="update-quantity" >
                                        <div className="D-quan-btn">
                                            ...
                                            <Button className="quan-btn" variant="contained" color='success'
                                                onClick={() => increment(item.type, index)}
                                            >
                                                +
                                            </Button>
                                        </div>
